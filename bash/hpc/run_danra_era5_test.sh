#!/bin/bash
#SBATCH --job-name=stride_danra_era5_test
...
# -----------------------------------------------------------------------------
# STRIDE DANRA/ERA5 test run on LUMI (inside container)
#
# Default behaviour:
...
#   bash/hpc/run_danra_era5_test.sh
#
# Optional overrides:
#   CONFIG_REL=configs/experiments/pipeline_w_shuffle.yaml
...
#   DANRA_ERA5_DATA_DIR=/scratch/project_465002493/$USER/Data/Data_DiffMod_small
# -----------------------------------------------------------------------------
...
DANRA_ERA5_DATA_DIR="${DANRA_ERA5_DATA_DIR:-${DATA_BASE}/Data_DiffMod_small}" # or Data_DiffMod for full dataset
...
CONFIG_REL="${CONFIG_REL:-configs/experiments/pipeline_w_shuffle.yaml}"
MODE="${MODE:-smoke}"
RUN_NAME="${RUN_NAME:-stride_danra_era5_test}"
RUN_ROOT="${RUN_ROOT:-${STRIDE_RUNS}/${RUN_NAME}}"
LOG_DIR="${LOG_DIR:-${RUN_ROOT}/logs}"

export ACCOUNT USER_BASE ROOT_DIR STRIDE_RUNS DATA_BASE DANRA_ERA5_DATA_DIR
...
echo "[INFO] DANRA_ERA5_DATA_DIR = ${DANRA_ERA5_DATA_DIR}"
...
if [ ! -d "${DANRA_ERA5_DATA_DIR}" ]; then
  echo "[WARNING] DANRA_ERA5_DATA_DIR does not exist: ${DANRA_ERA5_DATA_DIR}"
  echo "[WARNING] The dataset config must point to a valid DANRA/ERA5 directory on LUMI."
fi

echo "[INFO] Running STRIDE DANRA/ERA5 test inside container..."
...
srun singularity exec "${SINGULARITY_ARGS[@]}" "${CONTAINER}" bash -lc "
  ...
  export DANRA_ERA5_DATA_DIR='${DANRA_ERA5_DATA_DIR}'
